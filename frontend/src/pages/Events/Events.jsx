...
const Events = () => {
  ...
  const Track = ({track}) =>{
    ...
    const editTrack = async (id) =>{
      try {
        console.log(id);
        let response = await axios.put("http://127.0.0.1:8000/api/music/", {title, artist, album, genre, release_date});
        setEditing(false);
        getAllMusic();
      } catch (error) {
  ...
  const Event = ({event}) =>{
    ...
    const editEvent = async (id) =>{
      try {
        let response = await axios.put("http://127.0.0.1:8000/api/events/", {name, address, date, time});
        setEditing(false);
        // console.log(time1);
        getAllEvents();
    ...
    const removeEvent = async (id) =>{
      try {
        console.log(id);
        let response = await axios.delete("http://127.0.0.1:8000/api/events/");
        getAllEvents();
      } catch (error) {
        console.log(error.response.data);
      }
